...
class staff_menu(QMainWindow):
    def __init__(self, parent=None):
        ...
        staff_info_font.setFamily("G마켓 산스 TTF Light")
        ...
        staff_stock_font.setFamily("G마켓 산스 TTF Light")
...
class manager_stock_add(QMainWindow):
    def __init__(self, parent=None):
        ...
        brand_font.setFamily("G마켓 산스 TTF Light")
        ...
        kinds_font.setFamily("G마켓 산스 TTF Light")
        ...
        model_name_font.setFamily("G마켓 산스 TTF Light")
        ...
        price_font.setFamily("G마켓 산스 TTF Light")
        ...
        inventory_font.setFamily("G마켓 산스 TTF Light")
        ...
        brand_txt_font.setFamily("G마켓 산스 TTF Light")
        ...
        kinds_txt_font.setFamily("G마켓 산스 TTF Light")
        ...
        model_txt_font.setFamily("G마켓 산스 TTF Light")
        ...
        price_txt_font.setFamily("G마켓 산스 TTF Light")
        ...
        inventory_txt_font.setFamily("G마켓 산스 TTF Light")
...
class manager_stock_edit(QMainWindow):
    def __init__(self, parent=None):
        ...
        model_font.setFamily("G마켓 산스 TTF Light")
        ...
        inventory_num_font.setFamily("G마켓 산스 TTF Light")
...
class manager_stock_delete(QMainWindow):
    ...
    def model_combo_change(self):
        ...
        model_txtB_font.setFamily("G마켓 산스 TTF Light")
...
class First(QMainWindow):
    ...
    def send_btn_clicked(self):
        ...
        self.recv_txtB.append('소켓 생성')

        try:
            s.bind(('', 8888))
        except socket.error as msg:
            self.recv_txtB.append('바인드 실패. 에러 코드 : ' + str(msg[0]) + ' 메세지 : ' + msg[1])
        self.recv_txtB.append('바인드 성공')
        s.listen(10)
        self.recv_txtB.append('소켓 연결 준비중...')
        fl = 0
        ...
        self.recv_txtB.append('수신된 데이터 => ' + str(data.decode('utf8')))

        if not data:
            self.recv_txtB.append('사용할 수 없는 데이터입니다.')
            fl = 0

        if data == 'exit':
            self.recv_txtB.append('프로그램을 종료합니다.')
            breakpoint()

        # 안드로이드에서부터 0 수신 시 로그인된 정보 전송 (직책에 따라 메뉴가 다르기 때문)
        if data == '0':
            print("직책")

        # 안드로이드에서부터 1 수신 시 라즈베리파이로 모터 ON을 제어
        if data == '1':
            print("모터 ON")

        # 안드로이드에서부터 2 수신 시 라즈베리파이로 모터 OFF를 제어
        if data == '2':
            print("모터 OFF")

        # 안드로이드에서부터 3 수신 시 DB에 있는 상품의 정보를 전송
        if data == '3':
            print("상품 정보")

        # 안드로이드에서부터 4 수신 시 DB에 있는 직원 정보를 전송 (1줄에 직원의 이름, ID, 직책 등등 모두 전송)
        if data == '4':
            print("직원 정보")
    ...
    def database_btn_clicked(self):
        ...
        sql_update = "UPDATE user_info SET name = %s WHERE name = %s"
        # cur_update.execute(sql_update, ('오종', '오종진'))
        # con.commit()
        cur.execute(sql)
        # print(cur_update.rowcount)
        data = cur.fetchall()
        data_li = list(data)
        ...
        #
        # for i in len(data):
        #     # self.recv_txtB.append(str(data[i]))
        #     print(data)
